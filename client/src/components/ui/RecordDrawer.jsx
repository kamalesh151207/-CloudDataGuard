...
export default function RecordDrawer({ recordId, onClose }) {
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-950/60 backdrop-blur-sm flex justify-end">
      <div 
        className="w-full max-w-lg bg-[#0f172a] border-l border-slate-800 h-full p-6 overflow-y-auto flex flex-col justify-between shadow-2xl animate-in slide-in-from-right duration-300"
        ...
        <div>
          ...
          <div className="flex items-center justify-between border-b border-slate-800 pb-4">
            <div className="flex items-center space-x-3">
              <div className="w-10 h-10 rounded-xl bg-cyan-950/80 border border-cyan-800/60 text-cyan-400 flex items-center justify-center font-bold text-base">
                {record?.name ? record.name.charAt(0).toUpperCase() : 'R'}
              </div>
              <div>
                <h3 className="text-lg font-bold text-white tracking-tight">{record?.name || 'Loading...'}</h3>
                <p className="text-xs text-slate-400">Record ID: {recordId}</p>
              </div>
            </div>
            ...
            <div className="py-6 space-y-6">
              ...
              <div className="space-y-3">
                ...
                <div className="grid grid-cols-1 gap-2.5 text-xs">
                  <div className="p-3 rounded-xl bg-slate-850/60 border border-slate-800 flex items-center justify-between">
                    <span className="text-slate-400 flex items-center gap-2">
                      <Mail className="w-3.5 h-3.5 text-slate-400" /> Email
                    </span>
                    <span className="font-mono text-slate-200 font-medium">{record.email}</span>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-850/60 border border-slate-800 flex items-center justify-between">
                    ...
                    <span className="font-mono text-slate-200 font-medium">{record.phone}</span>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-850/60 border border-slate-800 flex items-center justify-between">
                    ...
                    <span className="text-slate-200 font-medium">{record.department}</span>
                  </div>
                  ...
                  <div className="p-3 rounded-xl bg-slate-850/60 border border-slate-800 flex items-center justify-between">
                    ...
                    <span className="text-slate-200 font-medium">{record.location}</span>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 rounded-xl bg-slate-900/80 border border-slate-800/80 space-y-2">
                <h4 className="text-xs font-bold text-slate-300 flex items-center gap-1.5">
                  <ShieldCheck className="w-4 h-4 text-emerald-400" /> Deduplication Result
                </h4>
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2 text-xs max-h-44 overflow-y-auto pr-1">
                    {record.history.map((log) => (
                      <div key={log._id} className="p-2.5 rounded-lg bg-slate-850 border border-slate-800 flex items-start space-x-2">
                        <span className={`w-2 h-2 rounded-full mt-1 flex-shrink-0 ${
                          log.status === 'verified' ? 'bg-emerald-400' : log.status === 'redundant' ? 'bg-rose-500' : 'bg-amber-400'
                        }`} />
                        <div>
