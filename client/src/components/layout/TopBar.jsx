    ...
    <header className="sticky top-0 z-20 bg-white/90 dark:bg-[#090d16]/90 backdrop-blur-md border-b border-slate-200 dark:border-slate-800/80 px-6 py-3.5 flex items-center justify-between transition-colors duration-200">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div 
          ...
          title="Click to refresh Supabase PostgreSQL health check"
        >
          <span className={`w-2 h-2 rounded-full ${isConnected ? 'bg-emerald-500 animate-pulse' : 'bg-rose-500'}`} />
          <Database className="w-3.5 h-3.5" />
          <span>{isConnected ? 'Supabase Connected' : 'DB Disconnected'}</span>
        </div>
        ...
        <div className="flex items-center p-1 rounded-xl bg-slate-100 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-800 text-xs">
          <button
            ...
            title="White & Blue Theme"
          >
            <Sun className="w-3 h-3 text-[#2563eb]" />
            <span className="hidden lg:inline">White & Blue</span>
          </button>
          ...
          <button
            ...
            title="AquaSmart AI Theme"
          >
            <Sparkles className="w-3 h-3 text-[#86efac]" />
            <span className="hidden lg:inline">AquaSmart</span>
          </button>
          ...
          <button
            ...
            className={`px-2.5 py-1 rounded-lg font-bold transition flex items-center gap-1 ${
              theme === 'cyberpunk'
                ? 'bg-gradient-to-r from-pink-600 to-cyan-500 text-white shadow-sm'
                : 'text-slate-500 dark:text-slate-400 hover:text-slate-800'
            }`}
